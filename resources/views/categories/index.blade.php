            ...
            <tbody>
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td><label class="switch">
                            <input onchange="published(this)" value="{{ $category->id }}" type="checkbox" <?php if($category->published == 1) echo "checked";?> >
                            <span class="slider round"></span></label></td>
                        <td>
...
@section('script')
    <script type="text/javascript">
        function published(el){
            ...
            $.post('{{ route('categories.published') }}', {_token:'{{ csrf_token() }}', id:el.value, published:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Category Published');
                }
                else{
